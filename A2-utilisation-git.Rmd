...
Une fois que vous avez démarré la SciViews Box à partir du lanceur rapide (ou directement à partir de VirtualBox), vous pourrez accéder à RStudio à partir du bouton correspondant dans la bannière rouge en haut de la page https://wp.sciviews.org. Un nouvel onglet s'ouvre dans votre borwser web (Chrome recommandé ici\ !) et le login vers RStudio apparaît. Il faut y entrer les informations suivantes\ :
...
Sur GitHub, vous pouvez héberger vos projets (qui se nomment sur GitHub `repositories`). Notre exemple se base sur le projet [sdd-umons-2020](https://github.com/BioDataScience-Course/sdd-umons-2020), que vous pouvez librement consulter. Il s'agit en effet d'un dépôt public. Vous avez la possibilité d'avoir des projets publics ou privés. 
...
Un dépôt sur GitHub est proposé de la manière suivante. Vous pouvez observer le nom du dépôt et la personne ou l'organisation qui travaille sur ce projet (`BioDataScience-Course/sdd-umons-2020`). Dans notre cas, tous les projets relatifs au cours de sciences des données biologiques sont hébergés sur [BioDataScience-Course](https://github.com/BioDataScience-Course)(Il en sera de même pour tous les travaux que vous réaliserez dans le cadre des cours). Sous le nom du dépôt, vous pouvez observer, le dépôts sur lequel ce projet s'inspire (`phgrosjean/bookdown-test`). 
...
Dans votre projet RStudio, sélectionnez l'onglet `Tools` puis `Shell...`. Un onglet **Terminal** vient de s'ouvrir à côte de l'onglet **Console** de R. Il vous suffit ensuite d'y copier les trois instructions proposée sur GitHub et de taper sur la touche `entrée` et le tour est joué.
