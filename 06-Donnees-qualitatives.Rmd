...
Pour de plus gros tableaux, il vaut mieux utiliser un tableur tel que Excel ou LibreOffice Calc pour l'encodage. Les tableurs en ligne comme Google Sheets ou Excel Online conviennent très bien également et facilitent un travail collaboratif ainsi que la mise à disposition sur le Net, comme nous avons vu au module \@ref(import).
...
`r h5p(52, height = 270, toc = "Tableau de contingences")`
...
`r h5p(51, height = 270, toc = "La fonction table()")`
...
`r h5p(54, height = 270, toc = "Précision et exactitudes")`
...
![`pivot_longer()` par [apreshill](https://github.com/apreshill/teachthat/blob/master/pivot/pivot_longer_smaller.gif).](images/sdd1_06/pivot_longer_smaller.gif)
...
`r h5p(55, height = 270, toc = "De long vers large")`
...
```{r}
shotgun_wide2 <- pivot_wider(shotgun_long, names_from  = batch, values_from = sequences) 
...
`r learnr("A06La_recombinaison", title = "Recombinaison de tableaux", toc = "Remodelage de tableaux  dans R")`
...
Pour empiler des tableaux l'un au dessus de l'autre, la fonction la plus simple est `bind_rows()`. Partons de données mesurées dans les mésoscosmes de notre laboratoire lors des travaux pratiques du cours d'océanographie générale. Les différentes variables mesurées sont les suivantes\ : 
...
Pour combiner des tableaux de données par les colonnes, de gauche à droite, la fonction la plus simple à utiliser est `bind_cols()`. Les étudiants ont également réalisé des prélèvements d'eau qui ont été dosés par colorimétrie avec un autoanalyseur. Les échantillons des deux groupes ont été analysés dans la même série par l'appareil, ce qui donne le tableau suivant pour les nutriments :
...
`r learnr("A06Lb_multi_tableau", title = "Traitements multi-tableaux", toc = "Traitements multi-tableaux dans R")`

##### A vous de jouer {-}

Vous avez à réaliser un projet libre par 3 sur la démographie en Belgique. Ce projet requiert d'avoir assimilé les 6 premiers module de ce cours.

```{r, echo=FALSE, results='asis'}
assignation("A06Ga_belgium_demo", part = NULL,
...
Vous venez de terminer le module 6. Ce module vous a permis d'apprendre à recombiner, remodeler et fusionner des tableaux. Pour évaluer votre compréhension de cette matière vous aviez les exercices suivants à réaliser\ :
